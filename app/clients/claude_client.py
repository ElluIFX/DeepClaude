...
class ClaudeClient(BaseClient):
    ...
    async def stream_chat(
        self, messages: list, model_arg: dict, model: str, stream: bool = True
    ) -> AsyncGenerator[tuple[str, str], None]:
        """流式或非流式对话

        Args:
            messages: 消息列表
            model_arg: 模型参数字典[temperature, top_p, presence_penalty, frequency_penalty]
        ...
        if self.provider == "openrouter":
            ...
            data = {
                ...
                "temperature": 1
                if model_arg["temperature"] < 0 or model_arg["temperature"] > 1
                else model_arg["temperature"],  # Claude仅支持temperature与top_p
                "top_p": model_arg["top_p"],
            }
        else:
